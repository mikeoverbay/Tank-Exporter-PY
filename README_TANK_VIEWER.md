...
✓ **Animated track chain** — per-side closed loop built each frame from drive sprocket + idler + road wheel + return roller hubs.  External-tangent math wraps each wheel at `R + segmentsInnerThickness`, drops wrong-side wraps to keep the spline from folding on suspension-compressed wheels, applies a per-pad polygon-correction tilt `δ/2 = atan(L/2 / R_eff)` so the pad face is ⊥ to the radial at the chord midpoint, and central-chord rebuilds each pad's forward tangent (`pos[i+1] − pos[i−1]`) so consecutive pads change angle by Δ/2.  Drive-sprocket tooth phase pulled from the chassis XML's `<teethSyncs><startAngle>` so one chain pad lands exactly on tooth k=0.  Two-piece pads (segment1 + segment2 on T110E4 etc.) share segment1's orientation matrix with segment2 shifted half a chord ahead.  Chain s_offset slaved to the drive sprocket angular advance (zero rim/chain slip).  Chassis settle uses `R_wheel + segmentsInnerThickness + segmentsOuterThickness` for wheel-centre Y so pad outer faces sit on the terrain.
