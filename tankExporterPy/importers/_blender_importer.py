...
def main():
    ...
    if ext == '.fbx':
        # use_custom_normals=True tells Blender to READ the per-loop
        # split normals from the FBX file instead of auto-recomputing
        # them.  Without this, smoothing groups + any custom normals
        # the FBX carries get clobbered by Blender's vertex-average
        # heuristic on import -- and the round-trip back to WoT then
        # ships the wrong normals.  Some Blender versions name this
        # option differently (`use_normal` / `auto_smooth_use`); we
        # try the canonical names in turn.
        try:
            bpy.ops.import_scene.fbx(
                filepath=args.input, use_custom_normals=True)
        except TypeError:
            try:
                bpy.ops.import_scene.fbx(
                    filepath=args.input, use_normal=True)
            except TypeError:
                bpy.ops.import_scene.fbx(filepath=args.input)
    elif ext in ('.glb', '.gltf'):
        # glTF's normal handling is bit-exact by default -- the
        # importer does not re-derive normals when the file declares
        # them.  No flag needed.
...
def _extract_mesh(bpy, obj, me):
    ...
    # ---- Positions (Blender Z-up, MESH-LOCAL space) --------------------
    positions = [(v.co.x, v.co.y, v.co.z) for v in me.vertices]

    # ---- Normals: per-loop CUSTOM SPLIT NORMALS, sampled per vertex ----
    # Reading me.vertices[i].normal would give Blender's auto-averaged
    # vertex-normal -- WRONG for round-trip because it discards any
    # smoothing-group / hard-edge / custom-normal data the FBX carried.
    # The proper source is the per-loop split-normal stream:
    #   * Blender 4.1+ : me.corner_normals[loop_index].vector
    #   * Blender 3.x / 4.0 : me.calc_normals_split() then
    #                          me.loops[loop_index].normal
    # Same first-loop-wins pattern we use for UVs: each vertex takes
    # the normal of the FIRST loop touching it.  Hard edges ARE
    # preserved across the round trip because the WoT pipeline stores
    # one normal per vertex anyway -- the choice of "which loop"
    # determines which side of the seam wins.
    normals    = [None] * len(me.vertices)
    use_corner = hasattr(me, 'corner_normals')
    if not use_corner:
        try:
            me.calc_normals_split()
        except AttributeError:
            pass     # Blender 4.1+: corner_normals already live
    for poly in me.polygons:
        for li in poly.loop_indices:
            ...
            if normals[vi] is None:
                ...
                normals[vi] = (float(n.x), float(n.y), float(n.z))
    # Defensive fallback: any orphan vertex (no incident loop -- rare
    # for well-formed meshes, possible for stray points after edits)
    # falls back to Blender's auto-vertex-normal.
    for i in range(len(normals)):
        if normals[i] is None:
            v = me.vertices[i].normal
            normals[i] = (float(v.x), float(v.y), float(v.z))
