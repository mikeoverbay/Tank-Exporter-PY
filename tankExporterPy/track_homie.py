...
def _filter_wrong_side_wraps(loop, max_iters=None):
    ...
    for _ in range(cap):
        ...
        for i in range(n):
            ...
            if dot < 0.0:
                # Wrong-side wrap.  Drop this wheel.  Sprockets
                # and idlers are normally exempt because they
                # carry the chain endpoints (losing the only
                # sprocket / idler on this side would collapse
                # the loop) -- but per Coffee 2026-06-04
                # ("trying to wrap an upper wheel that's an
                # idler") the exemption was too blanket.  The
                # v1.230.0 W_-singular reclassifier in
                # loaders.py can bucket non-leading W_ wheels
                # with road-comparable radii into `idlers_L/R`,
                # which means tanks with auxiliary upper
                # wheels end up with an EXTRA idler in the
                # role list.  When _filter_wrong_side_wraps
                # flagged that extra, the old code
                # unconditionally skipped it and the chain
                # dutifully tried to wrap an upper wheel that
                # had no business being in the loop.
                #
                # Endpoint-counting guard: drop a wrong-sided
                # sprocket / idler ONLY when there's at least
                # one OTHER wheel of the same role still in
                # the loop.  That preserves the original
                # "don't orphan the chain" guarantee for tanks
                # with a single sprocket / idler per side
                # (the common case), while letting the filter
                # remove misclassified extras.
                role = entry.get('wheel', {}).get('role', '')
                if role in ('sprocket', 'idler'):
                    same_role_count = sum(
                        1 for e in arcs
                        if e.get('wheel', {}).get('role', '')
                        == role)
                    if same_role_count <= 1:
                        # Only one of this role in the loop --
                        # leaving it in is the lesser evil.
                        continue
                drop_idx = i
                break
        if drop_idx is None:
