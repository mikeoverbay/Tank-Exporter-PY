...
substituted each wheel's chain-wrap radius with the
pad-centre radius `R_pad_centre = (R + outer) * cos(
asin((seg/2) / (R + outer)))`.  Reverted via `git revert`.

`track_homie.build_chain_segments` is back to no
`outer_thickness` arg; `viewer._chain_for_side` no longer
reads `segmentsOuterThickness` from chassis_info.  Chain
wraps each wheel at the authored `<groupRadius>` verbatim
(= v1.212.0 behaviour).

Skipped the 1.213.0 version number on the new bump because
the reverted-content version already held that slot in the
git history -- next bump landed at 1.214.0 to keep the
version timeline unambiguous.
