...
## See also (split out of this file as it grew)

| Topic | Doc |
|-------|-----|
| Track physics rewrite (NURB + bone-driven pads, in progress) | [`docs/TRACK_PHYSICS.md`](docs/TRACK_PHYSICS.md) |

Use `python cust_tools/arch.py search <topic>` to grep across every
doc in the repo and jump to the right section.  `python
cust_tools/arch.py list` enumerates every heading; `... stale` flags
docs that are older than the source files they describe.

---

<!-- The Track physics roadmap section that used to live here moved
     to docs/TRACK_PHYSICS.md on 2026-05-08.  As ARCHITECTURE.md
     keeps growing we'll continue extracting per-domain files
     (PHYSICS.md for tank_physics.py, RENDERING.md for viewer.py
     render passes, etc.) -- watch the "See also" table above. -->
